...
function NewPost({ onDone }: { onDone: ()=>void }) { const nav = useNavigate(); const [form, setForm] = useState({ type:'request', title:'', description:'', category:'errands', tags:'', lat:'37.7749', lon:'-122.4194', shareExact:false, redact:true, anonymous:true, radiusMiles:5 });
  return <Layout><form className="card space-y-2" data-testid="new-post-form" onSubmit={async(e)=>{e.preventDefault(); const lat=Number(form.lat), lon=Number(form.lon); const fuzzy=fuzzLocation(lat,lon); const post: Post = {id:uid(), type:form.type as Post['type'], title:form.title, description: form.redact ? redactPII(form.description):form.description, category:form.category, tags:form.tags.split(',').map(s=>s.trim()).filter(Boolean), lat: form.shareExact? lat: fuzzy.lat, lon: form.shareExact? lon: fuzzy.lon, exactLat: form.shareExact?lat:undefined, exactLon: form.shareExact?lon:undefined, shareExact: form.shareExact, radiusMiles:Number(form.radiusMiles), contactMethod:'in-app', author: form.anonymous?`Neighbor-${Math.floor(Math.random()*900+100)}`:'You', anonymous:form.anonymous, createdAt:new Date().toISOString(), updatedAt:new Date().toISOString()}; await db.posts.add(post); await appendAudit('create-post',{postId:post.id}); onDone(); nav('/feed'); }}>
